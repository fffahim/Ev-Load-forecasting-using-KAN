...
class SCINet(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
                num_levels = 3, num_decoder_layer = 1, concat_len = 0, groups = 1, kernel = 5, dropout = 0.5,
                 single_step_output_One = 0, input_len_seg = 0, positionalE = False, modified = True, RIN=False, device = 'cuda'):
    ...
    def get_position_encoding(self, x):
        max_length = x.size()[1]
        position = torch.arange(max_length, dtype=torch.float32, device= self.device)  # tensor([0., 1., 2., 3., 4.], device='cuda:0')
    ...
    def forward(self, x):
        ...
        if self.num_decoder_layer == 1:
            ...
            for div_projection in self.div_projection:
                output = torch.zeros(x.shape,dtype=x.dtype).to(self.device)
...
class Interactor(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.modified:
            ...
            # print("x even", x_even.shape)
            # print(self.phi(x_even).shape)
            # print("x odd", x_odd.shape)
